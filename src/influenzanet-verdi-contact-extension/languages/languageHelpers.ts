    ...
    LanguageHelpers.languages.forEach((translations, languageId) => {
      ...
      const get_trans = (key: string): [string, string] | undefined => {
        ...
        if (reference && translations[key][referenceKey] !== reference) {
          Logger.warn(
            `[WARNING] string ${key}[${languageId}] does not match survey definition!`
          );
          Logger.warn(
            `"${translations[key][referenceKey]}" !== "${reference}"\n`
          );
        }
...
export function _T(id: string, en?: string, alt?: string): LanguageMap {
  ...
  LanguageHelpers.languagesId.set(idKey, {
    ...idEntry,
    ...LanguageHelpers.languages.get(idKey),
  });

  return new LanguageMap([[idKey, id], ...t]);
}

export function _T_any(en: string): LanguageMap {
  return new LanguageMap([[anyKey, en]]);
}
